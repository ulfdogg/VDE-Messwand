...
class GPIOMonitor:
    ...
    def __init__(self, pin1=17, pin2=27, shutdown_timeout=120):
        ...
        # Optionaler Callback: wird aufgerufen wenn Notaus aktiviert wird
        # Signatur: on_notaus_active()
        self.on_notaus_active = None
            ...
            try:
                ...
                if current_state != last_state:
                    ...
                    if current_state:
                        ...
                        self.shutdown_triggered = False
                        # Callback auslösen (z.B. Sicherheitsrelais abschalten)
                        if self.on_notaus_active:
                            try:
                                self.on_notaus_active()
                            except Exception as cb_err:
                                print(f"⚠️ Callback-Fehler bei Notaus: {cb_err}")
                    else:
...
def update_gpio_shutdown_timeout(seconds: int):
    """Aktualisiert den Shutdown-Timeout am laufenden GPIO-Monitor"""
    if gpio_monitor is not None:
        gpio_monitor.shutdown_timeout = seconds
