...
def get_training_relays_with_details(page_id, category):
    """
    Gibt konfigurierte Relais einer Kategorie/Übungsseite mit ihren Details zurück.
    Fallback: wenn keine training_config-Einträge vorhanden, alle Relais der Kategorie
    aus der relais_config nehmen.

    Args:
        page_id: ID der Übungsseite (z.B. "spannungsfrei")
        category: Kategorie (z.B. "RISO")

    Returns:
        Liste von Dicts mit relay_num, name, stromkreis, category
    """
    from managers.relais_manager import load_relais_config, get_relais_by_category

    relay_nums = get_relais_for_training(page_id, category)

    # Fallback: wenn keine training_config vorhanden, alle Relais der Kategorie verwenden
    if not relay_nums:
...
def import_from_relais_manager(category, page_id):
    ...
    """
    try:
        from managers.relais_manager import get_relais_by_category
